...
sub repack_boot {
	...
	my %args = (base => "0x10000000", kernel_offset => "0x00008000", ramdisk_offset => "0x01000000", second_offset => "0x00f00000", tags_offset => "0x00000100", pagesize => 2048, board => "", cmdline => $ENV{BOARD_KERNEL_CMDLINE});
	...
	system ("$tool --kernel $kernel --ramdisk temp-$ramdiskfile --cmdline \"$ENV{BOARD_KERNEL_CMDLINE}\" @extrargs -o $outfile");
