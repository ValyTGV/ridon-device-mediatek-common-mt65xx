...
MTKM4UDrv::MTKM4UDrv(void)
{
    ...
    mFileDescriptor = open("/dev/M4U_device", O_RDONLY);
    if(mFileDescriptor<0)
    {
        M4UMSG("XOpen file failed mFileDescriptor=%d, error=%d :%s", mFileDescriptor, errno, strerror(errno));
        m4u_dump_backtrace();
    }
    else
    {
        ...
        {
            M4UMSG(" Xioctl MTK_M4U_T_M4UDrv_CONSTRUCT fail! fd=%d, %d, %s \n", mFileDescriptor, errno, strerror(errno));
            m4u_dump_backtrace();
        }
